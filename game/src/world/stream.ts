...
type Chunk={batch:Batch;ground:T.Mesh;body:RAPIER.RigidBody;colliders:number;detail:boolean};
...
export class WorldStream {
  ...
  readonly groundMaterial:T.MeshStandardMaterial;
  readonly farGround:T.Mesh;readonly skyline:T.Group;
  private farBatch:Batch;private lastCenter='';
  ...
  constructor(readonly scene:T.Scene,readonly physics:RAPIER.World,readonly materials:Materials,readonly data:WorldData,public quality:Settings['quality']){
    const sand=materials.values.sand;
    this.groundMaterial=worldUV(new T.MeshStandardMaterial({vertexColors:true,roughness:1,metalness:0,map:sand.map,normalMap:sand.normalMap,roughnessMap:sand.roughnessMap}),5);
    this.groundMaterial.normalScale.set(.28,.28);
    ...
    for(let i=0;i<p.count;i++){
      ...
      p.setXYZ(i,x,terrainHeight(x,z)-.15,z);const c=new T.Color(isLand(x,z)?0x95a47e:0x376e70);colors.push(c.r,c.g,c.b);
    }
    ...
    this.farBatch=new Batch(this.primitives,materials,0,0,true);
    ...
    // Distant proxies are masked wherever full chunks own the same cells.
  ...
  private make(cx:number,cz:number):Chunk {
    ...
    for(let i=0;i<p.count;i++){
      const x=p.getX(i)+ox,z=p.getZ(i)+oz,y=terrainHeight(x,z);p.setY(i,y);
      const beach=x>2007&&x<2320&&z>-3400&&z<700;
      const n=(hash(Math.floor(x/8),Math.floor(z/8))%100)/1500;
      const c=new T.Color(beach?0xfff1d2:y<0?0x436d65:0x95a57b);c.offsetHSL(0,0,n);colors.push(c.r,c.g,c.b);
    }
    g.setAttribute('color',new T.Float32BufferAttribute(colors,3));g.computeVertexNormals();
    ...
    for(const s of SEGMENTS){
      ...
      for(let i=0;i<count;i++){
        ...
        const crossing=SEGMENTS.some(other=>other.id!==s.id&&Math.abs(Math.sin(other.yaw-s.yaw))>.3&&Math.hypot(x-other.a[0],z-other.a[1])+Math.hypot(x-other.b[0],z-other.b[1])<other.length+3);
    ...
    // Inferred infill never moves sourced landmark pins.
    ...
    for(let z=Math.floor(oz/24)*24;z<oz+256;z+=24){
      if(z>-1450&&z<330)for(const x of [2012,2046])if(this.inside(x,z,cx,cz)){
        palm(batch,x,z,2.4,hash(x,z));if(z%72===0)bench(batch,x+4,z,2.4);
      }
    }
    ...
    return {batch,ground,body,colliders,detail:this.quality!=='low'};
  }
  private retire(c:Chunk){this.physics.removeRigidBody(c.body);c.batch.dispose();c.ground.removeFromParent();c.ground.geometry.dispose();}
  ...
  update(x:number,z:number,budget=1){
    ...
    if(center!==this.lastCenter){
      this.lastCenter=center;const plan=planStreaming(this.chunks,needed,this.quality!=='low');
      for(const key of plan.remove){this.retire(this.chunks.get(key)!);this.chunks.delete(key);}
      this.pending=plan.pending;changed=true;
    }
    for(let n=0;n<budget&&this.pending.length;n++){
      const cell=this.pending.shift()!,old=this.chunks.get(cell.key),fresh=this.make(cell.x,cell.z);
      if(old)this.retire(old);this.chunks.set(cell.key,fresh);changed=true;
    }
    if(changed)this.farBatch.hideCells(new Set(this.chunks.keys()),CHUNK_SIZE);
  }
  get stats(){return {chunks:this.chunks.size,pending:this.pending.length,detailedChunks:[...this.chunks.values()].filter(c=>c.detail).length,colliders:[...this.chunks.values()].reduce((s,c)=>s+c.colliders,0)};}
  dispose(){for(const c of this.chunks.values())this.retire(c);this.chunks.clear();this.farBatch.dispose();this.farGround.geometry.dispose();this.farGround.removeFromParent();this.groundMaterial.dispose();this.primitives.dispose();}
}
