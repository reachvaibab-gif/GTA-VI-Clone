...
export class Vehicle {
  ...
  constructor(readonly physics:RAPIER.World,scene:T.Scene,materials:Materials,x:number,z:number,yaw:number,color:T.ColorRepresentation,readonly police=false){
    this.body=physics.createRigidBody(RAPIER.RigidBodyDesc.dynamic().setTranslation(x,surfaceHeight(x,z)+.85,z).setRotation({x:0,y:Math.sin(yaw/2),z:0,w:Math.cos(yaw/2)}).enabledRotations(false,true,false).setLinearDamping(.1).setAngularDamping(3).setCcdEnabled(true));
    this.collider=physics.createCollider(RAPIER.ColliderDesc.cuboid(.92,.55,2.05).setMass(1250).setFriction(.18).setRestitution(.12),this.body);
  ...
  drive(dt:number,throttle:number,steering:number,handbrake=false){
    ...
    let acceleration=throttle*(throttle*long<-.5?13:6.2)-long*.04-long*Math.abs(long)*.0018;
    ...
    this.body.resetForces(true);this.body.addForce({x:fx*acceleration*1250,y:0,z:fz*acceleration*1250},true);
    ...
    this.lastSpeed=long;
    this.model.brake.emissiveIntensity=handbrake||throttle*long<-.3?2.4:.4;
  }
  follow(dt:number,x:number,z:number,targetSpeed=14){
    ...
    const throttle=clamp((targetSpeed-this.speed)*.35,-1,1);
    this.drive(dt,throttle,clamp(delta*1.9,-1,1));
  }
  render(dt:number,night:number){
    ...
    this.model.wheels.forEach((wheel,i)=>{wheel.rotation.y=i<2?this.steer:0;});
    this.model.spins.forEach(spin=>spin.rotation.x+=this.speed*dt/.365);
